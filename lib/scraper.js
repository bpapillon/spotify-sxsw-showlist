  ...
  request(url, function(error, response, html){
    if (error) {
      ...
      var $ = cheerio.load(html),
          ...
          shows = [],
          elm, elm_id, node, artist, location, time;
      for (var i = 0; i < nodes.length; i++) {
        elm_id = nodes[i] && nodes[i].attribs && nodes[i].attribs.id;
        if (!elm_id || elm_id.substr(0, 11) !== 'cell_event_') {
          continue;
        }
        node = $(nodes[i]);
        elm = node.find('.link_item');
        artist = elm && elm.text().trim();
        if (!artist) {
          continue;
        }
        elm = node.find('.location');
        location = (elm && elm.text() || '').trim() || 'TBA';
        elm = node.find('.date_time');
...
Scraper.unofficial = function(day, res) {
  ...
  _.each(dayParts, function(dayPart){
    ...
    var url = 'http://showlistaustin.com/sxsw/' + year + '/' + day.substr(0,3) + dayPart + '.shtml';
    request(url, function(error, response, html){
      if (error) {
        loadedCount++;
        return Scraper._error(error);
